...
After that is complete, we construct a "AnalysisDataManager" object, which is responsible for running the desired step. the data of the analysis. This is a very [simple class](https://github.com/defog-ai/defog-self-hosted/blob/4f23350480624c47c5fe75459a4987d8068e4033/backend/analysis_data_manager.py#L31) that just stores the data in a dictionary, and calls the aforementioned post process functions + all agent's functions.
...
We first call the `run_agent` [function](https://github.com/defog-ai/defog-self-hosted/blob/4f23350480624c47c5fe75459a4987d8068e4033/backend/analysis_data_manager.py#L158) of AnalysisDataManager, which:
...
- Then, it initialises the required agent: either `clarifier` or `planner_executor` [here](https://github.com/defog-ai/defog-self-hosted/blob/4f23350480624c47c5fe75459a4987d8068e4033/backend/analysis_data_manager.py#L203-L209).
