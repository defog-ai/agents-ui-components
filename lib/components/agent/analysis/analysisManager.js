...
function AnalysisManager({
  ...
  async function deleteSteps(stepIds) {
    const res = await deletestepIds(analysisId, stepIds, apiEndpoint);
              ...
              if (idx > -1) {
                ...
                if (requestType === "gen_steps" && res.id) {
                  ...
                  if (updatedData.success) {
                    newtoolRunDataCache[updatedData.id] = updatedData;
                  }
                }
              } else {
  ...
  async function onReRunSocketMessage(event) {
    ...
    try {
      ...
      if (response.success) {
        newtoolRunDataCache[response.id] = Object.assign({}, response);
      }
