...
export function AnalysisTreeManager(initialTree = {}, id = v4()) {
  ...
  function updateAnalysis({ analysisId, isRoot, updateObj, emit = true }) {
    ...
    // else find the root analysis and update the child
    const rootAnalysis = newAnalysisTree[analysisId];
    rootAnalysis.analysisList = rootAnalysis.analysisList.map((item) => {
