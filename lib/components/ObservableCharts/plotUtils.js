...
export const getMarks = (data, mergedOptions) => {
  ...
  if (mergedOptions.facet) {
    return [
      ...
      ...baseMarks,
      mergedOptions.type !== "histogram" && Plot.ruleY([0]),
      getYAxis(mergedOptions),
      ...chartMarks,
    ];
  } else {
    return [...baseMarks, ...chartMarks];
  }
};
...
const getBarMarks = (data, mergedOptions) => {
  return [
    Plot.barY(data, {
      x: mergedOptions.x,
      y: Array.isArray(mergedOptions.y) ? mergedOptions.y[0] : mergedOptions.y,
...
const getHistogramMarks = (data, mergedOptions) => {
  const { x, binCount, fillColor, thresholds, normalize, cumulative } =
    mergedOptions;
  ...
  return [
    Plot.rectY(
      ...
      Plot.binX(
        ...
        {
          x: x,
          thresholds: thresholds === "auto" ? binCount : thresholds,
          fill: fillColor,
          normalize: normalize,
