...
import { getConn, runQueryOnDb } from "../../utils/sqlite";
 ...
 *
 * @param {object} config
 * @param {string} config.analysisId - The id of the analysis
 * @param {string} config.apiEndpoint - The endpoint for the api
 * @param {object} config.mainSocket - The main websocket
 * @param {object} config.rerunSocket - The rerun websocket
 * @param {function} config.onNewData - Callback for new data
 * @param {function} config.onReRunData - Callback for rerun data
 * @param {function} config.onManagerDestroyed - Callback for when the manager is destroyed
 * @param {string} config.token - token for verficiation with defog servers
 * @param {string} config.keyName - api key name
 * @param {boolean} config.devMode - dev mode
 * @param {object} config.metadata - metadata for the database
 * @param {boolean} config.isTemp - is this a temporary database (sqlite + csv)
 * @param {object} config.createAnalysisRequestBody - request body for creating an analysis. This will be sent to the backend
 * @param {boolean} config.sqlOnly - if this is a sql only analysis
 * @returns
 */
function AnalysisManager({
  ...
  async function init({ question, existingData = null, sqliteConn }) {
    ...
    // update existingAnalysisData with the created analysisData
      ...
      // currently metadata is in the format:
      // [{
      //   column_description
      // column_name
      // data_type
      // table_name
      // }]
      // we need to convert this to the format that the api expects
      // {
      // "table_name": [
      //   {
      ...
      // ]
      // }
      // also the table_name will be d.sqlite_table_name

      const sqliteTableName = metadata[0].sqlite_table_name;

      const newMetadata = {
        [sqliteTableName]: metadata.map((d) => {
          return {
            column_name: d.column_name,
            data_type: d.data_type,
            column_desc: d.column_description,
          };
        }),
      };
      ...
      // run query on sqlite
      const result = runQueryOnDb({ conn: sqliteConn, query: res.sql });

      // get the columns of this table from the sqlite db
      // i want them in the same order as the returned output
      let columns = runQueryOnDb({
        conn: sqliteConn,
        query: `PRAGMA table_info(${sqliteTableName})`,
      }).map((d) => d[1]);
