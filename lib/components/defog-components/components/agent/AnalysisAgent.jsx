...
export const AnalysisAgent = ({
  ...
  const { mainManager, reRunManager, toolSocketManager } =
    socketContext.val.socketManagers;
    ...
    async function initialiseAnalysis() {
      try {
        ...
        if (analysisManager.wasNewAnalysisCreated) {
          // also have to set socketContext in this case
          socketContext.update({
            ...socketContext.val,
            userItems: {
              ...socketContext.val.userItems,
              analyses: [...socketContext.val.userItems.analyses, analysisId],
            },
          });
        }
        if (
