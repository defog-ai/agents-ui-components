...
export function AnalysisTreeItem({
  ...
  return (
    ...
    >
      <div className="grow">
        <div
          className={isDummy ? "p-3 hover:cursor-pointer flex items-center" : twMerge(
        ...
        >
          {isDummy ? (
            <div className="flex items-center">
              <SquarePen size={18} /> <span className="font-medium ml-2">New Report</span>
            </div>
          ) : (
            <>
              <div className="line-clamp-2 font-medium">{sentenceCase(analysis?.user_question)}</div>
              {analysis?.timestamp && (
