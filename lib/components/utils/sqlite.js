...
export const addParsedCsvToSqlite = ({ conn, tableName, rows, columns }) => {
  ...
  for (const row of rows) {
    ...
    runQueryOnDb({ conn, query: insertStmt, bind: rowValues });
  }
  ...
  const columnMetadata = columns.map((c) => {
    ...
    const columnType = runQueryOnDb({ conn, query: columnTypeStmt });
  ...
  const fiveRowsAsArraysOfValues = runQueryOnDb({ conn, query: selectStmt });
