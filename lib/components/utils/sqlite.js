...
/**
 * Cleans table name for SQLite.
 *
 * @param {string} tableName
 * @returns {string} - Cleaned table name.
 *
 * @example
 * const tableName = cleanTableNameForSqlite("My Table-123.0"); // "my_table_123"
 *
 * const tableName = cleanTableNameForSqlite("location 2.01.01 PM"); // location_2.01.01_pm
 *
 *
 */
export const cleanTableNameForSqlite = (tableName) => {
...
/**
 * Cleans column name for SQLite.
 *
 * @param {string} colName
 * @returns {string} - Cleaned column name.
 */
export const cleanColumnNameForSqlite = (colName) => {
 ...
 * @returns {{columns:Array<string>, rows: Array<Array<any>> error: string | null}} [columns, rows] Column names and array of rows.
 */
...
export const addParsedCsvToSqlite = ({ conn, tableName, rows, columns }) => {
  ...
  const columnMetadata = columns.map((c) => {
    ...
    const columnType = runQueryOnDb({ conn, query: columnTypeStmt }).rows;
  ...
  const fiveRowsAsArraysOfValues = runQueryOnDb({
    conn,
    query: selectStmt,
  }).rows;
