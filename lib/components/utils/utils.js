 ...
 * @param {string} params.apiEndpoint - API endpoint
 * @param {Array<string>} params.previousQuestions - Previous questions
 *
 ...
 *
 * @returns {Promise<{ success: boolean, error_message?: string, question_type?: string }>}
 */
...
export async function getQuestionType(token, keyName, apiEndpoint, question) {
  ...
  if (!response.ok) {
    ...
    return { success: true, question_type: responseJson["question_type"] };
  }
}
