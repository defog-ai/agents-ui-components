...
import { reFormatData } from "../defog-components/common/utils";
import Papa from "papaparse";

export const getAnalysis = async (reportId, apiEndpoint) => {
  const urlToConnect = setupBaseUrl(protocol="http", path="get_report", apiEndpoint=apiEndpoint);
  let response;
  try {
...
export const createAnalysis = async (
  ...
  const urlToConnect = setupBaseUrl(protocol="http", path="create_analysis", apiEndpoint=apiEndpoint);
  let response;
  try {
    response = await fetch(urlToConnect, {
      method: "POST",
...
export const getAllDocs = async (token, keyName, apiEndpoint) => {
  const urlToConnect = setupBaseUrl(protocol="http", path="get_docs", apiEndpoint=apiEndpoint);
  let response;
  try {
    response = await fetch(urlToConnect, {
      method: "POST",
...
export const getTableData = async (tableId, apiEndpoint) => {
  const urlToConnect = setupBaseUrl(protocol="http", path="get_table_chart", apiEndpoint=apiEndpoint);
  let response;
  try {
    response = await fetch(urlToConnect, {
      method: "POST",
...
export const getAllAnalyses = async (keyName, apiEndpoint) => {
  const urlToConnect = setupBaseUrl(protocol="http", path="get_analyses", apiEndpoint=apiEndpoint);
  let response;
  try {
    response = await fetch(urlToConnect, {
      method: "POST",
...
export const getToolboxes = async (token) => {
  const urlToConnect = setupBaseUrl(protocol="http", path="get_toolboxes", apiEndpoint=apiEndpoint);
  let response;
  try {
    response = await fetch(urlToConnect, {
      method: "POST",
...
export const getToolRunData = async (toolRunId, apiEndpoint) => {
  const url = setupBaseUrl(protocol="http", path="get_tool_run", apiEndpoint=apiEndpoint);
  let response;
  try {
...
export const deleteDoc = async (docId, apiEndpoint) => {
  const url = setupBaseUrl(protocol="http", path="delete_doc", apiEndpoint=apiEndpoint);
  let response;
  try {
    response = await fetch(url, {
      method: "POST",
...
export const addTool = async ({
  ...
  const addToolEndpoint = setupBaseUrl(protocol="http", path="add_tool", apiEndpoint=apiEndpoint);
  const payload = {
    tool_name,
    function_name,
    description,
...
export function parseCsvFile(file, cb) {
  ...
  Papa.parse(file, {
    ...
    complete: (results) => {
      ...
      rows = rows.map((row, i) => ({
        key: i,
        ...Object.fromEntries(columns.map((col, j) => [col.dataIndex, row[j]])),
      }));

      cb({ parsedData, rows, columns });
    },
  });
}
